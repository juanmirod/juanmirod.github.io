...
El paper en sí tiene su importancia y valor histórico, en el momento en el que salió modelos com BERT y más tarde GPT3 o LaMDA estaban dando bastante que hablar y las autoras se cuestionan ciertas preguntas éticas sobre el uso de esos modelos, su utilidad, su consumo y la (falta de) planificación y rigor en su entrenamiento. La argumentación de que estos modelos "sólo tratan de predecir el siguiente token" es aun así algo controvertida. Para predecir el siguiente token, de alguna forma tienes que saber que es lo que pone hasta ese momento y qué es lo más probable a continuación. No es una cuestión de simple estadística o de sintaxis, de alguna forma hay que codificar la semántica. Y sobre cómo se codifica la semántica en en el espacio geométrico de miles de dimensiones que tienen los llms hay muchas fuentes, dejo por aquí la más ilustrativa que conozco, que es este video corto de 3brown1blue: <https://www.youtube.com/shorts/FJtFZwbvkI4>
...
Sin embargo los modelos de ahora tienen muchas más fases de entrenamiento, se ha mejorado la calidad de los datos, se ha introducido entrenamiento por refuerzo automático además del entrenamiento con refuerzo humano en tareas concretas.

(post aún incompleto, continuaré describiendo estas fases)
